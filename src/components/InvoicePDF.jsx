...
const styles = StyleSheet.create({
  ...
  col2: { width: '35%', fontSize: 9, color: '#374151' },
  col3: { width: '12%', textAlign: 'center', fontSize: 9, color: '#374151' },
  col4: { width: '13%', textAlign: 'right', fontSize: 9, color: '#374151' },
  col5: { width: '15%', textAlign: 'right', fontSize: 9, color: '#059669' },
  col6: { width: '20%', textAlign: 'right', fontSize: 9, color: '#1f2937', fontWeight: 'bold' },
...
// PDF Document Component with GST
      ...
      {invoiceData.items && invoiceData.items.length > 0 && (
        <View style={styles.table}>
          <View style={styles.tableHeader}>
            ...
            <Text style={[styles.col5, styles.colHeader]}>GST Amt</Text>
            <Text style={[styles.col6, styles.colHeader]}>Amount</Text>
          </View>
          {invoiceData.items.map((item, index) => (
            <View key={index} style={index % 2 === 0 ? styles.tableRow : styles.tableRowAlt}>
              ...
              <Text style={styles.col5}>{formatINR(item.gstAmount)}</Text>
              <Text style={styles.col6}>{formatINR(item.amount)}</Text>
            </View>
          ))}
        </View>
      )}
...
export default function InvoiceGenerator() {
  ...
  const updateItem = useCallback((index, field, value) => {
    setInvoiceData(prev => {
      ...
      const numValue = ['quantity', 'rate', 'gst'].includes(field) ? parseFloat(value) || 0 : value;
      newItems[index][field] = numValue;
      
      if (field === 'quantity' || field === 'rate' || field === 'gst') {
        const baseAmount = newItems[index].quantity * newItems[index].rate;
        const gstAmount = (baseAmount * newItems[index].gst) / 100;
  ...
  const addFirstItem = useCallback(() => {
    setInvoiceData(prev => ({
      ...prev,
      items: [{ name: '', quantity: 1, rate: 100, gst: 18, gstAmount: 18, amount: 118 }]
    }));
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-purple-600 to-pink-600 text-white hidden md:table-header-group">
                    <tr>
                      ...
                      <th className="px-4 py-4 text-center font-semibold w-24">
                        <Percent className="w-4 h-4 inline mr-1" />
                        GST
                      </th>
                      <th className="px-4 py-4 text-right font-semibold w-32">GST Amt</th>
                  ...
                  <tbody>
                    {invoiceData.items.map((item, index) => (
                      <tr key={index} className={`block md:table-row border-b ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-purple-50 transition`}>
                        ...
                        <td className="block md:table-cell px-4 py-3 md:py-3 md:text-center" data-label="GST %">
                          <span className="font-bold text-gray-600 md:hidden">GST %: </span>
                          <input
                            type="number"
                            value={item.gst}
                            onChange={(e) => updateItem(index, 'gst', e.target.value)}
                            className="w-full md:w-24 border-2 border-green-300 rounded-lg px-3 py-2 text-left md:text-center focus:border-green-500 focus:outline-none transition bg-green-50 mt-1 md:mt-0"
                          />
                        </td>
                        <td className="block md:table-cell px-4 py-3 md:py-3 text-left md:text-right" data-label="GST Amount">
                          <span className="font-bold text-gray-600 md:hidden">GST Amount: </span>
                          <span className="font-semibold text-green-600">{formatINR(item.gstAmount)}</span>
                        </td>
              ...
              <button
                onClick={() => setInvoiceData(prev => ({
                  ...prev,
                  items: [...prev.items, { name: '', quantity: 1, rate: 100, gst: 18, gstAmount: 18, amount: 118 }]
                }))}
